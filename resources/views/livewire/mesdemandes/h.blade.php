<div>
    <h2 class="titre_page"> Mes Demandes</h2>
    <br>
    <div class="filter">
        ...
        <button class="recent" title="Inverser l'ordre de tri">
        ...
        <div id="div_class_search">
            ...
            <div class="inputsearch">
                <input type="text" wire:model.defer.debounce.250ms="searchdemande" placeholder="Search ... " id="mysearch">
            </div>
            <span class="clear" id="clear" style="dispaly:none;"></span>
        </div>
    </div>
        ...
        <div class="tous_filtres" id="id_filters">
            ...
            <div class="deux_filtres">
                <div class="filter_popup1">
                    <input class="cherche" wire:model='filtredemandeur' list="user1" name="cherche_demandeur" placeholder="     Demandeur...">
                    <datalist id="user1" wire:model='filtredemandeur'>
                        @foreach ($Users as $user)
                            @if ($user['user_nom']==0)
                                <option value={{$user['user_nom']}}>
                            @endif
                ...
                <div class="filter_popup1">
                    <input class="cherche" wire:model='filtreanalyseur' list="user2" name="cherche_analyseur" placeholder="     Analyseur...">
                    <datalist id="user2" wire:model='filtreanalyseur'>
                        @foreach ($Users as $user)
                            @if ($user['user_role']==1)
            ...
            <div class="deux_filtres">
                <div class="filter_popup1">
                    <input class="cherche" wire:model='filtrevalideur' list="user3" name="cherche_valideur" placeholder="     Valideur...">
                    <datalist id="user3" wire:model='filtrevalideur'>
                        @foreach ($Users as $user)
                            @if ($user['user_role']==2)
                                <option value={{$user['user_nom']}} {{$user['user_prenom']}}>
                            @endif
                ...
                <div class="filter_popup1">
                    <input class="cherche" wire:model='filtrecodeur' list="user4" name="cherche_codeur" placeholder="     Codeur...">
                    <datalist id="user4" wire:model='filtrecodeur'>
                        @foreach($Users as $user)
                            @if($user['user_role']==3)
                                <option value={{$user['user_nom']}} {{$user['user_prenom']}}>
                            @endif
            ...
            <div class="deux_filtres">
                <div class="filter_popup2" >
                    <select name="Filtre1" class="list" wire:model='filtredepartement'>
            ...
            <div class="filter_popup2">
                <select name="Filtre5" class="list" wire:model='filtreprotocole'>
                    <option value="hide" class="select-hidden">Protocole</option>
                    @foreach ($Protocoles as $protocole)
                        <option value={{$protocole['pro_nom']}}>{{$protocole['pro_nom']}}</option>
                    @endforeach
                </select>
            </div>
            <div class="deux_filtres">
                ...
                <div class="filter_popup3">
                    ...
                    <br>
                    <input class="date" wire:model='filtredatedemande' type="date" name="datedemande" id="">
                </div>
            </div>
    ...
    @if($affichage==false)
        @foreach($mesdemandes as $mademande)
            @if($mademande->archivee==0)
                @if ($mademande->annulee==0)
                    <div class="une_demandes">
                        <div class="check_arch">
                            <input type="checkbox" name="" class="check_box" >
                            <div class="div_centrer" title="Archiver">
                                <svg class="archives" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                <path 
            ...
            @if($mademandefiltre->archivee==0)
                @if ($mademandefiltre->annulee==0)
                    <div class="une_demandes">
                        ...
                        <p>{{$mademandefiltre->created_at}}</p>
                        <div class ="div_centrer" title="Afficher les détails" id="id_info">
                            <svg class="more_infos" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                <path 
                                    d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM216 336h24V272H216c-13.3 0-24-10.7-24-24s10.7-24 24-24h48c13.3 0 24 10.7 24 24v88h8c13.3 0 24 10.7 24 24s-10.7 24-24 24H216c-13.3 0-24-10.7-24-24s10.7-24 24-24zm40-208a32 32 0 1 1 0 64 32 32 0 1 1 0-64z"/>
    ...
    @endif
    <button class="bouton" id="btn1" wire:click="confirmAjoutDemande({{auth()->user()->id}})">
        Ajouter une Demande
    </button>
...
<script>
        ...
        document.addEventListener('click', function o_c_search(event) {
            // 👇️ the element the user clicked
            console.log('user clicked: ', event.target);
            const search = document.getElementById('div_class_search');
            const fermer = document.getElementById('clear');
            if (search.contains(event.target)) {
                search.style.width = '200px';
                fermer.style.display = 'block';
            }
            if(fermer.contains(event.target)) {
